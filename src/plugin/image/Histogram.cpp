...
vector<float> Histogram::getRelativeHistogram(vector<int> h)
{
	...
	maxValue = *max_element(h.begin(), h.end()); //Les valeurs de l'histogramme relatif sont calculees en fonction de la valeur maximale de l'histogramme absolu.
...
void Histogram::drawTooltip()
{
	for (int i = 0; i < histogramRelative->size(); i++)
	{
		...
		{
			std::string text = "Valeur de couleur: " + ofToString(i) + '\n' +
